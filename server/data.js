...
export async function create(resource, input, user, options = {}) {
  ...
  if (resource === 'payments') {
    ...
    if (input.id) { const byId = await prisma.payment.findUnique({ where: { id: clean(input.id) } }); if (byId) { const byIdParcel = await prisma.parcel.findUnique({ where: { id: byId.parcelId } }); if (!byIdParcel || !canEditPayment(user, byId, byIdParcel)) throw new Error('Forbidden.'); return publicValue(byId); } }
    try {
