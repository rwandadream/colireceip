...
export function TripNewPage() {
    ...
    const submit = async (event: React.FormEvent) => { event.preventDefault(); if (!form.trip_number.trim() || vehicles.some((vehicle) => !vehicle.registration.trim())) return; setSaving(true); const trip = await createTrip({ ...form, trip_number: form.trip_number.trim(), created_by: user?.id || '', created_by_name: user?.full_name || '' }); for (const vehicle of vehicles) await createTripVehicle({ trip_id: trip.id, registration: vehicle.registration.trim(), ...Object.fromEntries(feeFields.map((field) => [field, Number(vehicle[field]) || 0])) } as never); navigate(`/trips/${trip.id}`); };
    return <div className="max-w-4xl mx-auto space-y-5"><div className="flex items-center gap-3"><Link to="/trips" className="p-2"><ArrowLeft size={20} /></Link><div><h1 className="text-2xl font-bold text-slate-900 dark:text-white">Nouveau voyage</h1><p className="text-sm text-slate-500">Créer le trajet puis lui affecter ses véhicules.</p></div></div>
        <form onSubmit={submit} className="space-y-5"><Card className="p-5"><div className="grid gap-4 md:grid-cols-2"><Input label="Numéro de voyage" placeholder="001" value={form.trip_number} onChange={(e) => setForm({ ...form, trip_number: e.target.value })} required /><Input label="Date" type="date" value={form.trip_date} onChange={(e) => setForm({ ...form, trip_date: e.target.value })} required /><Input label="Départ" value={form.origin} onChange={(e) => setForm({ ...form, origin: e.target.value })} required /><Input label="Destination" value={form.destination} onChange={(e) => setForm({ ...form, destination: e.target.value })} required /><Select label="Statut" value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value as typeof form.status })}><option value="planned">Planifié</option><option value="in_transit">En route</option><option value="arrived">Arrivé</option><option value="closed">Clôturé</option><option value="cancelled">Annulé</option></Select></div></Card>
            ...
            {vehicles.map((vehicle, index) => <Card key={index} className="p-5"><div className="flex justify-between gap-3 mb-4"><h3 className="font-semibold">Véhicule {index + 1}</h3>{vehicles.length > 1 && <button type="button" onClick={() => setVehicles(vehicles.filter((_, i) => i !== index))} className="text-error-600"><Trash2 size={17} /></button>}</div><Input label="Immatriculation réelle" value={vehicle.registration} onChange={(e) => updateVehicle(index, 'registration', e.target.value)} required /><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{feeFields.map((field) => <Input key={field} label={`${feeLabels[field]} (FCFA)`} type="number" min={0} value={vehicle[field]} onChange={(e) => updateVehicle(index, field, e.target.value)} />)}</div></Card>)}
