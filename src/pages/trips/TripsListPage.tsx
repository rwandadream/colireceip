...
export function TripsListPage() {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {trips.map((trip) => { const stat = stats[trip.id] || { vehicles: 0, parcels: 0, expenses: 0 }; return <Link key={trip.id} to={`/trips/${trip.id}`}>
        <Card className="p-5 h-full hover:border-brand-300 transition-colors"><div className="flex justify-between gap-3"><div><p className="text-lg font-bold text-slate-900 dark:text-white">Voyage {trip.trip_number}</p><p className="text-sm text-slate-500">{trip.origin} → {trip.destination}</p></div><span className="badge bg-brand-50 text-brand-700">{getTripStatusLabel(trip.status)}</span></div>
