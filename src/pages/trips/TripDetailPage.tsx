...
function getVehicleFees(vehicle: TripVehicle): number {
    return feeGroups.reduce((total, group) => total + group.fields.reduce((subtotal, field) => subtotal + vehicle[field], 0), 0);
}
...
export function TripDetailPage() {
    ...
    const deleteVehicle = async (vehicle: TripVehicle) => { await deleteTripVehicle(vehicle.id); await load(); };
    const setStatus = async (status: Trip['status']) => { const updated = await updateTrip(trip.id, { status }); if (updated) setTrip(updated); };
